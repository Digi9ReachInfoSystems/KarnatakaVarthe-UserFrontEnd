...
export default function NewsHero({ districtSlug, dateFilter = null }) {
  ...
  useEffect(() => {
    if (rawData.length > 0) {
      ...
      // Take only the first 3 latest news items for hero section
      const latestNews = sortedNews.slice(0, 3);
      
      const normalized = latestNews.map((it, i) => {
        const normalizedId = it._id?.$oid || it._id || it.id || `api-${i}`;
        return {
          _id: String(normalizedId),
          id: String(normalizedId),
          title: (it[langKey]?.title?.slice(0, 50) ?? it.title?.slice(0, 50) ?? " ") + "...",
          excerpt: (it[langKey]?.description?.slice(0, 150) ?? it.description?.slice(0, 150) ?? " ") + "...",
    ...
    >
      <HeroMedia>
        ...
        <img 
          ...
          aria-hidden="true" 
          loading="lazy"
          onClick={() => navigate(`/districtnewsdetails/${current._id}`)}
          style={{ cursor: 'pointer' }}
        />
      </HeroMedia>
      <OverlayCard>
          <Title as="h3" onClick={() => navigate(`/districtnewsdetails/${current._id}`)} style={{ cursor: 'pointer' }}>{current.title}</Title>
